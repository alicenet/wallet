        ...
        return function handleAction(action) {
            switch (action.type) {
                case MIDDLEWARE_ACTION_TYPES.INIT_ALICENET_WALLET:
                    return initAliceNetWallet(
                        action.payload,
                        storeAPI.dispatch
                    );
                case MIDDLEWARE_ACTION_TYPES.ADD_WALLET_FROM_KEYSTORE:
                    return addWalletFromKeystore(
                        action.payload.data,
                        action.payload.name,
                        storeAPI.dispatch,
                        storeAPI.getState
    ...
    if (!address) {
        address =
            curve === curveTypes.SECP256K1
                ? await util.wallet.getSecp256k1FromPrivKey(pKey)
                : await util.wallet.getBNfromPrivKey(pKey);
    }
...
function initAliceNetWallet(initPayload, dispatch) {
    return new Promise(async (res) => {
        log.debug(
            "Initiating AliceNetWallet Instance & Synchronizing to Redux State. . ."
        );
        ...
        for (let walletType in initPayload.wallets) {
            for (let wallet of initPayload.wallets[walletType]) {
                if (walletType === "internal") {
                    internalAccountAdds.push(
                        await _walletArrayStructure(
                            wallet.privK,
                            wallet.curve,
                            wallet.name,
                            wallet.address
                    ...
                    // else, is an external wallet
                    externalAccountAdds.push(
                        await _walletArrayStructure(
                            wallet.privK,
                            wallet.curve,
                            wallet.name,
        ...
        try {
            ...
            allToAdd.forEach((addition) => {
                addedPromises.push(
                    (async () => {
                        return new Promise(async (res) => {
                            try {
                                await aliceNetWallet.Account.addAccount(
                                    addition[0],
                                    addition[1]
                                );
                                res(true);
                                ...
                                if (
                                    ex.message ===
                                    "Account.addAccount: Account already added"
                                ) {
                                    log.warn(
                                        "Wallet " +
                                            addition[2] +
                                            " already exists in the AliceNetJS Instance."
                                    );
                                    res({
                                        error:
                                            "Wallet " +
            ...
            Promise.all(addedPromises).then(async (values) => {
                ...
                let balancedState = await buildBalancedWalletState(
                    internalAccountAdds,
                    externalAccountAdds
                );
                log.debug(
                    "AliceNetWalletInit SUCCESS :: Dispatching set vault state "
                );
                dispatch({
                    type: VAULT_ACTION_TYPES.SET_WALLETS_STATE,
                    payload: balancedState,
                });
                res([true, errors]);
            });
        } catch (ex) {
...
function addWalletFromKeystore(keystore, walletName, dispatch, getState) {
    return new Promise(async (res) => {
        ...
        if (
            curve !== curveTypes.SECP256K1 &&
            curve !== curveTypes.BARRETO_NAEHRIG
        ) {
            throw new Error(
                "An invalid curve was found within a keystore: " + curve
            );
        }
        ...
        let pKeyMatches = wallets.filter(
            (wallet) => wallet.privK === util.wallet.strip0x(pKey)
        );
        if (pKeyMatches.length > 0) {
            res({
                error:
                    "This private key is already loaded as wallet: " +
                    pKeyMatches[0].name,
            });
        }
        ...
        try {
            ...
            let address =
                curve === curveTypes.BARRETO_NAEHRIG
                    ? await utils.wallet.getBNfromPrivKey(pKey)
                    : await utils.wallet.getSecp256k1FromPrivKey(pKey);
            let externalAdds = [
                await _walletArrayStructure(
                    util.wallet.strip0x(pKey),
                    curve,
                    walletName,
                    address
                ),
...
function addNextHDWallet(storeAPI, walletName) {
    return new Promise(async (res) => {
        ...
        let nextHdWallet =
            await utils.wallet.streamLineHDWalletNodeFromMnemonic(
                mnemonic,
                nextDerrivationPath
            );
        // Generate internal wallet object
        let walletObj = await utils.wallet.generateBasicWalletObject(
            walletName,
            nextHdWallet.privateKey.toString("hex"),
            desiredCurve
        );
        // Add the private key to the AliceNetJs instance
        try {
            ...
            let internalAdds = [
                await _walletArrayStructure(
                    util.wallet.strip0x(walletObj.privK),
                    walletObj.curve,
                    walletObj.name
                ),
            ];
...
function buildBalancedWalletState(internalAdds, externalAdds) {
    return new Promise(async (res) => {
        ...
        for (let account of aliceNetWallet.Account.accounts) {
            let signerKeyToUse =
                parseInt(account.curve) === 1 ? "secpSigner" : "bnSigner"; // Key to use under MultiSigner for this account to get privK
            ...
            let matches = allToAdd.filter(
                (addition) => addition[0] === privK && addition[3] === address
            );
            ...
            if (matches.length > 1) {
                throw Error(
                    "During balancing, a private key & address combo was filtered twice. This signals a duplicate wallet. Verify state balancing does not allow multiple matches per iteration. Check all forwarded paramaters and current state."
                );
            }
            ...
            if (!match) {
                log.warn(
                    "No match found for recently added wallet list against a AliceNetJs Account List -- This is normal during additions up to the current wallet count, only matches need balanced."
                );
                continue;
            }
            let walletObj = util.wallet.constructWalletObject(
                ...
                account.address,
                signerKeyToUse === "bnSigner"
                    ? util.wallet.curveTypes.BARRETO_NAEHRIG
                    : util.wallet.curveTypes.SECP256K1,
                internalAdds.filter((addition) => addition[0] === privK)
                    .length === 1 // is this an internal wallet?
            );
...
export function removeWallet({ wallets, targetWallet, optout, exists }) {
    return new Promise(async (res) => {
        ...
        if (targetWallet.isInternal) {
            if (
                exists &&
                internalWallets.findIndex(
                    (w) => w.address === targetWallet.address
                ) === 0
            ) {
                res({
                    error: "If a Vault exists, main internal wallet must not be removed",
                });
            } else {
                internalWallets = internalWallets.filter(
                    (w) => targetWallet.address !== w.address
                );
            }
        }
        // Else external...
            ...
            if (
                optout &&
                externalWallets.findIndex(
                    (w) => w.address === targetWallet.address
                ) === 0
            ) {
                res({
                    error: "If user has opted out, main external wallet must not be removed",
                });
            } else {
                externalWallets = externalWallets.filter(
                    (w) => targetWallet.address !== w.address
                );
            }
        }
