...
export function setExistingMnemonic(phrase) {
    if (typeof phrase !== "string") {
        try {
            ...
            log.error(
                "Unable to parse passed parameter to string. Seed Phrase must be passed as string or simple array!"
            );
        }
    }
    return async function (dispatch) {
        dispatch({
            type: USER_ACTION_TYPES.SET_POTENTIAL_SEED_PHRASE,
            payload: phrase,
        });
    };
}
...
export function setDesiredCurveType(curveType) {
    return async function (dispatch) {
        if (
            curveType !== curveTypes.SECP256K1 &&
            curveType !== curveTypes.BARRETO_NAEHRIG
        ) {
            throw new Error(
                "Attempting to dispatch setDesiredCurveType with invalid curve type. Use utils.curveTypes! "
            );
        }
        dispatch({
            type: USER_ACTION_TYPES.SET_DESIRED_HD_CURVE,
            payload: curveType,
        });
    };
}
...
export function initialUserAccountCheck() {
    return async function (dispatch, getState) {
        ...
        if (vaultState.is_locked !== null || vaultState.exists !== null) {
            log.warn(
                "Attempting to run initialUserAccountCheck() again. Why? Isolate and remove duplicate calls to this function. ONLY IN DEBUG IS THIS NORMAL :: Opening debug will generate this warning.\n\nENV DEBUG MODE : " +
                    utils.generic.isDebug
            );
            return { vault: vaultState.exists, optout: vaultState.optout };
        }
        // Initial Check . . .
        let vaultExists =
            await electronStoreCommonActions.checkIfUserHasVault();
        let optOutExists =
            await electronStoreCommonActions.checkForOptoutStores();
        if (vaultExists) {
            ...
            // Else set clear state
            log.debug(
                "Neither vault or optout collections exists! Setting state to match."
            );
            dispatch({
                type: VAULT_ACTION_TYPES.MARK_NONEXIST_NONLOCK_NONOPTOUT,
            });
