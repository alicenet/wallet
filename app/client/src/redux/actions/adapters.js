...
export const setWeb3Connected = (isConnected) => {
    return (dispatch) => {
        dispatch({
            type: ADAPTER_ACTION_TYPES.SET_WEB3_CONNECTED,
            payload: isConnected,
        });
    };
};
...
export const setWeb3Info = (epoch, validators, max_validators) => {
    return (dispatch) => {
        dispatch({
            type: ADAPTER_ACTION_TYPES.SET_WEB3_INFO,
            payload: {
                epoch: epoch,
...
export const setWeb3Busy = (busyState) => {
    return (dispatch) => {
        dispatch({
            type: ADAPTER_ACTION_TYPES.SET_WEB3_BUSY,
            payload: busyState,
        });
    };
};
...
export const setAliceNetBusy = (busyState) => {
    return (dispatch) => {
        dispatch({
            type: ADAPTER_ACTION_TYPES.SET_ALICENET_BUSY,
            payload: busyState,
        });
    };
};
...
export const setAliceNetConnected = (isConnected) => {
    return (dispatch) => {
        dispatch({
            type: ADAPTER_ACTION_TYPES.SET_ALICENET_CONNECTED,
            payload: isConnected,
        });
    };
};
...
export const setAliceNetError = (error) => {
    return (dispatch) => {
        dispatch({
            type: ADAPTER_ACTION_TYPES.SET_ALICENET_ERROR,
            payload: error,
        });
    };
};
...
export const setAliceNetKeyChainValue = (keyChain, value) => {
    return (dispatch) => {
        dispatch({
            ...
            payload: { keyChain: keyChain, value: value },
        });
    };
};
...
export const initAdapters = () => {
    return async (dispatch, getState) => {
        ...
        let aliceNetConnected = await dispatch(
            initAliceNet({ preventToast: true })
        ); // Attempt to init aliceNetAdapter -- Adapter will handle error toasts
        ...
        if (web3Connected.error && aliceNetConnected) {
            ...
            let wallets = [
                ...getState().vault.wallets.internal,
                ...getState().vault.wallets.external,
            ];
            if (wallets.length === 0) {
                return { success: true };
            }
            ...
            return {
                error: "Unable to initiate both adapters -- Verify network configuration, and check property errors for error collection if needed on subsequent runs.",
                errors: {
    ...
    return async (dispatch, getState) => {
        ...
        if (web3Connected) {
            ...
            try {
                ...
                log.warn(
                    "Unable to parse wallet data for web3 balances of address: " +
                        address,
                    ". Is there a state imbalance? Error: ",
                    ex
                );
            }
        }
        // Push an IIFE for a false resolving promise to position 0, to maintain array positions for the Promise.all resolve
            ...
            log.debug(
                "Skipping eth/staking/util balance fetch for address: " +
                    address +
                    " :: Web3 not connected."
            );
        }
        ...
        if (aliceNetConnected) {
            balancePromises.push(
                aliceNetAdapter.getAliceNetWalletBalanceWithUTXOsForAddress(
                    address
                )
            );
        } else {
            ...
            log.debug(
                "Skipping aliceNetBytes/UTXO balance fetch for address: " +
                    address +
                    " :: AliceNet not connected."
            );
        }
        ...
        if (
            typeof foundBalances[0] !== "undefined" &&
            foundBalances[0] !== false &&
            !foundBalances[0].error
        ) {
            addressBalances.eth = foundBalances[0].balances.eth;
            addressBalances.stake =
                foundBalances[0].balances.stakingToken.balance;
            addressBalances.stakeAllowance =
                foundBalances[0].balances.stakingToken.allowance;
            addressBalances.util =
                foundBalances[0].balances.utilityToken.balance;
            addressBalances.utilAllowance =
                foundBalances[0].balances.utilityToken.allowance;
        }
        // AliceNet Bytes/UTXOs
        if (
            typeof foundBalances[1] !== "undefined" &&
            !foundBalances[1].error
        ) {
        ...
        dispatch({
            type: VAULT_ACTION_TYPES.SET_BALANCES_STATE,
            payload: updatedBalanceState,
        });
...
export const getAndStoreRecentTXsForAddress = (address, curve) => {
    return async (dispatch) => {
        ...
        let [txs, currentBlock] = await aliceNetAdapter.getPrevTransactions([
            {
                //eslint-disable-line
                address: address,
                curve: curve,
        ...
        dispatch({
            type: VAULT_ACTION_TYPES.UPDATE_RECENT_TXS_BY_ADDRESS,
            payload: {
                address: address,
                txs: txs,
            },
        ...
        log.debug(
            "Recent TXs fetched and updated to state for address: ",
            address
        );
        return true;
    };
...
const _prepTxObjectsToAliceNetAdapter = async (state) => {
    ...
    txReducerTxs.forEach((tx) => {
        if (tx.type === transactionTypes.DATA_STORE) {
            preppedTxObjs.push(
                transactionUtils.createDataStoreObject(
                    tx.from,
                    tx.key,
                    tx.value,
                    tx.duration
            ...
            preppedTxObjs.push(
                transactionUtils.createValueStoreObject(
                    tx.from,
                    tx.to,
                    tx.value,
                    tx.bnCurve
            ...
            throw new Error(
                "sendTransactionReducerTXs received incorrect txType of type: ",
                tx.type
            );
        }
    });
    ...
    log.debug(
        "Prepped TX Objects To AliceNetAdapter (Not in AliceNetJS Instance yet) => ",
        {
...
export const createAndClearFakeTxForFeeEstimates = () => {
    return async (dispatch, getState) => {
        ...
        if (
            !state.transaction.feePayer.wallet ||
            state.transaction.list.length === 0
        ) {
            return false;
        }
        ...
        log.debug(
            "AliceNetAdapter FAKE_TxOuts(ForFeeEstimates) pending: ",
        ...
        await aliceNetAdapter
            .wallet()
            .Transaction.createTxFee(
                feePayerWallet.address,
                feePayerWallet.curve,
                txFee
            );
...
export const sendTransactionReducerTXs = () => {
    return async (dispatch, getState) => {
        ...
        log.debug(
            "AliceNetAdapter TxOuts pending: ",
            aliceNetAdapter.txOuts.get()
        );
        ...
        if (create.error) {
            return {
                error: "Unable to create transaction: " + String(create.error),
            };
        }
        // If OK, send it
        ...
        try {
            ...
            for (let i = 0; i < txDatas.vouts.length; i++) {
                ...
                let extractedOwner = (
                    await aliceNetWalletInstance.Transaction.Utils.extractOwner(
                        vout.owner
                    )
                )[2];
            ...
            log.error(
                "Unable to parse returned tx for owners to aggregate balances. Did tx fail? -- Error: " +
                    ex
            );
        }
