...
} from "redux/reducers/configuration";
...
export function loadDefaultValues() {
    return async function (dispatch) {
        ...
        electronStoreCommonActions.storeConfigurationValues(
            initialConfigurationState
        );
 ...
 */
export function saveConfigurationValues({
    chainId,
    aliceNetProvider,
    ethProvider,
    ...
    return async function (dispatch, getState) {
        ...
        try {
            let updateObject = {
                alice_net_chainID: useFallbackValueForUndefinedInput(
                    chainId,
                    currentConfig.alice_net_chainID
                ),
                alice_net_provider: useFallbackValueForUndefinedInput(
                    aliceNetProvider,
                    currentConfig.alice_net_provider
                ),
                ethereum_provider: useFallbackValueForUndefinedInput(
                    ethProvider,
                    currentConfig.ethereum_provider
                ),
                registry_contract_address: useFallbackValueForUndefinedInput(
                    registryContractAddress,
                    currentConfig.registry_contract_address
                ),
                advanced_settings: useFallbackValueForUndefinedInput(
                    enableAdvancedSettings,
                    currentConfig.advanced_settings
                ),
                hide_generic_tooltips: useFallbackValueForUndefinedInput(
                    hideGenericTooltips,
                    currentConfig.hide_generic_tooltips
                ),
                has_seen_tx_help_modal: useFallbackValueForUndefinedInput(
                    hasSeenTxHelpModal,
                    currentConfig.has_seen_tx_help_modal
                ),
            };
            ...
            dispatch({
                type: CONFIG_ACTION_TYPES.SAVE_CONFIGURATION,
...
export function loadConfigurationValuesFromStore() {
    return async function (dispatch) {
        try {
            let loadedConfig =
                await electronStoreCommonActions.readConfigurationValues();
            if (
                loadedConfig.error === "Key is not in secure-electron-storage!"
            ) {
                log.debug(
                    "Configuration doesn't exist -- Using initial configuration state"
                );
                return false;
            ...
            dispatch({
                type: ADAPTER_ACTION_TYPES.SET_WEB3_ERROR,
                payload: false,
            });
            dispatch({
