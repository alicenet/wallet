...
export default function Web3Panel() {
    ...
    const walletArrayToButtons = (walletArray, actionFunction) => {
        return walletArray.map((wallet) => {
            let isDisabled = !activeWallet
                ? false
                : utils.wallet.compareAddresses(
                      wallet?.address,
        ...
        <>
            <Segment>
                <Header as="h4">
                    Web3 Overview
                    <Header.Subheader>
                        Debug web3 instance initiated:{" "}
                        {String(!!web3AdapterState.connected)}
                    </Header.Subheader>
                </Header>
                <div className="flex justify-between items-end">
                    <div>
                        ...
                        <span className="ml-8">
                            ...
                            {String(web3AdapterState.error)}
                        </span>
                        <span className="ml-8">
                            <span className="font-bold">validators:</span>{" "}
                            {String(web3AdapterState.validators) +
                                "/" +
                                String(web3AdapterState.max_validators)}
                        </span>
                        <span className="ml-8">
                            <span className="font-bold">epoch:</span>{" "}
                            {String(web3AdapterState.epoch)}
                        </span>
                    </div>
                    <div>
                        <Button.Group size="mini">
                            <DButton
                                ...
                                content={
                                    !web3AdapterState.connected
                                        ? "Init Web3 Adapter"
                                        : "Print web3Adapter Instance"
                                }
                                onClick={
                                    !web3AdapterState.connected
                                        ? initWeb3Adapter
                                        : () => console.log(web3Adapter)
                                }
                            />
                        </Button.Group>
                    </div>
            ...
            <Segment>
                ...
                <Button.Group widths="equal" size="mini">
                    {walletArrayToButtons(
                        wallets.internal,
                        addWalletToWeb3Instance
                    )}
                </Button.Group>
                ...
                <div className="flex justify-between items-end">
                        ...
                        <div>
                            <span>
                                ...
                                {utils.string.splitStringWithEllipsis(
                                    String(activeWallet.address),
                                    4
                                )}
                            </span>
                            <span className="ml-8">
                                <span className="font-bold">ethBal:</span>{" "}
                                {String(activeWallet?.balances?.eth)}
                            </span>
                            <span className="ml-8">
                                ...
                                {String(
                                    activeWallet?.balances?.stakingToken
                                        ?.balance
                                ) +
                                    "/" +
                                    String(
                            ...
                            <span className="ml-8">
                                ...
                                {String(
                                    activeWallet?.balances?.utilityToken
                                        ?.balance
                                ) +
                                    "/" +
                                    String(
                    ...
                    <div>
                        <Button.Group size="mini">
                            <DButton
                                content="Print active wallet"
                                onClick={() => console.log(activeWallet)}
                            />
                        </Button.Group>
                    </div>
                </div>
