...
export const initialTransactionState = {
    ...
    fees: {
        // Human readable fees -- The fees to be used by the adapter are in adapter reducer as HEX values
        isLoading: false,
...
export default function transactionReducer(
    ...
    switch (action.type) {
        ...
        case TRANSACTION_ACTION_TYPES.TOGGLE_STATUS:
            ...
            return Object.assign({}, state, {
                status: utils.transaction.getNextTransactionStatus(
                    state.status
                ),
            });
        ...
        case TRANSACTION_ACTION_TYPES.UPDATE_FEES_BY_TYPE:
            ...
            if (!action.payload) {
                throw new Error(
                    "UPDATE_FEES_BY_TYPE called without payload -- Always pass a payload to this action."
                );
            }
            return Object.assign({}, state, {
                fees: {
                    isLoading:
                        typeof action.payload.isLoading !== "undefined"
                            ...
                            : state.fees.isLoading,
                    atomicSwapFee:
                        typeof action.payload.atomicSwapFee !== "undefined"
                            ? action.payload.atomicSwapFee
                            : state.fees.atomicSwapFee,
                    atomicSwapFees:
                        typeof action.payload.atomicSwapFees !== "undefined"
                            ? action.payload.atomicSwapFees
                            : state.fees.atomicSwapFees,
                    dataStoreFee:
                        typeof action.payload.dataStoreFee !== "undefined"
                            ? action.payload.dataStoreFee
                            : state.fees.dataStoreFee,
                    dataStoreFees:
                        typeof action.payload.dataStoreFees !== "undefined"
                            ? action.payload.dataStoreFees
                            : state.fees.dataStoreFees,
                    depositFees:
                        typeof action.payload.depositFees !== "undefined"
                            ? action.payload.depositFees
                            : state.fees.dataStoreFees,
                    valueStoreFee:
                        typeof action.payload.valueStoreFee !== "undefined"
                            ? action.payload.valueStoreFee
                            : state.fees.valueStoreFee,
                    valueStoreFees:
                        typeof action.payload.valueStoreFees !== "undefined"
                            ? action.payload.valueStoreFees
                            : state.fees.valueStoreFees,
                    minTxFee:
                        typeof action.payload.minTxFee !== "undefined"
                            ? action.payload.minTxFee
                            : state.fees.minTxFee,
                    prioritizationFee:
                        typeof action.payload.prioritizationFee !== "undefined"
                            ? action.payload.prioritizationFee
                            : state.fees.prioritizationFee,
                    txFee:
                        typeof action.payload.txFee !== "undefined"
                            ? action.payload.txFee
                            : state.fees.txFee,
                    totalFee:
                        typeof action.payload.totalFee !== "undefined"
                            ? action.payload.totalFee
                            : state.fees.totalFee,
                    errors:
                        typeof action.payload.errors !== "undefined"
        ...
        case TRANSACTION_ACTION_TYPES.REMOVE_FROM_LIST:
            log.debug(
                "Removing an element from the list at position",
                action.payload
            );
            return Object.assign({}, state, {
