...
function NotificationIcon({ notifications }) {
    ...
    if (hasNotifications) {
        ...
        return (
            <Icon name="bell" className="animate-pulse text-red-500 group-hover:text-black" />
        )
    }
        ...
        <Popup
            ...
            content={
                <Container className="w-64">
                    <div className="text-right cursor-pointer" onClick={() => setIsOpen(false)}>
                        <Icon name="close" className="mx-0" />
                    </div>
                    {/*We are only showing the first notification */}
                    <div>
                        <Notification notification={notifications[0]} onClose={() => setIsOpen(false)} />
                    </div>
                </Container>
            }
