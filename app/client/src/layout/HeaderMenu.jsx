...
function HeaderMenu({ showMenu }) {
    ...
    const exemptLockLocations = [
        "/returningUserLoad/hasKeystore",
        "wallet/settings",
        "wallet/advancedSettings",
    ];
    const pathIsLockExempt = () => {
        // The lock should not appear on any page in exemptLockLocations
    ...
    const activeTabPane = (() => {
        if (pathname.indexOf("/hub") !== -1) {
            return tabPaneIndex.Wallets;
        }
        if (pathname.indexOf("/transactions") !== -1) {
            return tabPaneIndex.Transactions;
        }
        return tabPaneIndex.Wallets;
    })();
                ...
                <div className="flex">
                    {showMenu && existingAccount && (
                        <>
                            <MenuTabItem
                                name="Wallets"
                                activeId={tabPaneIndex.Wallets}
                                gotoPath="/hub"
                            />
                            <MenuTabItem
                                name="Transactions"
                                activeId={tabPaneIndex.Transactions}
                                gotoPath="/transactions"
                            />
                        </>
                    )}
                </div>
                        ...
                        <Popup
                            ...
                            trigger={
                                <Menu.Item
                                    ...
                                    header
                                    onClick={() =>
                                        dispatch(VAULT_ACTIONS.lockVault())
                                    }
                                    onKeyDown={(e) =>
                                        (e.key === " " ||
                                            e.key === "Enter" ||
                                    ...
                                    className="group px-3 hover:bg-transparent"
                                    role="button"
                                    tabindex="0"
                                    aria-pressed="false"
                                >
                        ...
                        <Popup
                            ...
                            trigger={
                                ...
                                >
                                    <Icon
                                        name="cog"
                                        className="transform duration-300 group-hover:rotate-90"
                                    />
                                </Menu.Item>
                            }
                        />
