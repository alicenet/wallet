...
export default function SearchTXs() {
    ...
    const updateTxHashVal = (val) =>
        setTxHashVal((s) => ({ ...s, value: val }));
    const updateTxHashErr = (err) =>
        setTxHashVal((s) => ({ ...s, error: err }));

    const { polledTxs } = useSelector((s) => ({
        polledTxs: s.transaction.polledTxs,
    }));
    ...
    const txPerPage = 12;
    const totalPages =
        Math.ceil(polledTxs.length / txPerPage) === 0
            ? 1
            : Math.ceil(polledTxs.length / txPerPage);
    const [activePage, setPage] = React.useState(0);
    const pageForward = () => setPage((s) => s + 1);
    const pageBackward = () => setPage((s) => s - 1);

    const activeSlice = polledTxs.slice(
