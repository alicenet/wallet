...
export default function AddEditPrioritizationFeeModal() {
    ...
    const { minFee } = useSelector((state) => ({
        minFee: state.transaction.fees.minTxFee,
    }));
    ...
    const handleSubmit = async () => {
        dispatch(
            TRANSACTION_ACTIONS.setPrioritizationFee(
                parseInt(formState.Fee.value)
            )
        );
        setOpenModal(false);
    };
    ...
    const [formState, formSetter, onSubmit] = useFormState([
        {
            name: "Fee",
            display: "AliceNet Bytes Fee",
            type: "integer",
            isRequired: true,
            value: prioritizationFee,
        },
    ]);
    ...
    return (
        <Modal
            ...
            trigger={
                <Menu.Item
                    as={Button}
                    name="add-prioritization-fee"
                    disabled={txList.length === 0}
                    onClick={() => setOpenModal(true)}
                >
                    <Icon name="chain" className="text-gray-600" />
                    Adjust Tx Fee
                </Menu.Item>
            }
        >
                            ...
                            <Grid className="m-0 content-evenly gap-2">
                                <Grid.Row>
                                    <Grid.Column width="16">
                                        <p>
                                            AliceNet Transactions cost a minimum
                                            fee. <br />
                                            You may provide additional
                                            AliceNetBytes to give your
                                            transaction priority. <br />
                                        </p>
                                    </Grid.Column>
                                </Grid.Row>
