...
function InspectionModule() {
    ...
    if (!tx.error) {
        ...
        txFee = Web3.utils
            .toBN(String(get(txObj, ["wholeTx", "Fee"], 0)))
            .toString();
    }

    // Need a state for the async owner extraction
    const [voutOwners, setVoutOwners] = useState(
        Array(get(txObj, "voutCount", 0))
    );
    ...
    useEffect(() => {
        const getOwners = async () => {
            ...
            for (let i = 0; i < txObj.voutCount; i++) {
                let idxOwner = await aliceNetJs.Utils.extractOwner(
                    txObj.vouts[i].owner
                );
        ...
        <div className="flex items-center">
            ...
            <div className="ml-3">
                <Label
                    ...
                    content={
                        isString
                            ? utils.generic.hexToUtf8Str(value)
                            : parseInt(value, 16).toLocaleString()
                    }
                />
            </div>
        </div>
            ...
            return Object.keys(focusVin).map((key) => {
                ...
                return (
                    <Table.Row key={`row-detail-${key}`}>
                        <Table.Cell style={{ borderRight: "1px solid silver" }}>
                            {stringUtils.prettifyUnderscoreKey(key)}
                        </Table.Cell>
                        ...
                        <Table.Cell className="flex items-center border-l-0">
                            ...
                            value &&
                            value.length > 52
                                ? stringUtils.splitStringWithEllipsis(value, 52)
                                : value}
                        </Table.Cell>
                    </Table.Row>
                );
        ...
        return (
            <>
                <Header sub className="text-xs m-0 flex items-center">
                    <div className="flex justify-between items-center m-auto w-full">
                        <Button
                            ...
                            size="mini"
                            onClick={() =>
                                setVinFocus((vinFocus) => vinFocus - 1)
                            }
                        />
                        <div className="text-xs text-gray-600 mr-1">
                            VINs {vinFocus + 1} of {totalPages}{" "}
                        </div>
                        <Button
                            disabled={vinFocus >= totalPages - 1}
                            icon="right chevron"
                            size="mini"
                            onClick={() =>
                                setVinFocus((vinFocus) => vinFocus + 1)
                ...
                <Table
                    ...
                    size="small"
                    compact
                    className="text-xs m-0 my-2"
                >
                    <Table.Body>{genRows()}</Table.Body>
                </Table>
            </>
        );
    };
    ...
    const VoutTable = () => {
                ...
                return (
                    <Table.Row key={`row-detail-${key}`}>
                        ...
                        <Table.Cell className="flex items-center">
                            ...
                            value &&
                            value.length > 70 ? (
                                stringUtils.splitStringWithEllipsis(value, 70)
                            ) : // Owner key parsing
                            key === "owner" ? (
                                <>
                                    {voutOwners &&
                                        voutOwners[voutFocus] &&
                                        voutOwners[voutFocus][2]}
                                    {voutOwners &&
                                    voutOwners[voutFocus] &&
                                    voutOwners[voutFocus][1] === 2 ? (
                                        <Label
                                            size="mini"
                                            color="blue"
                                            className="ml-4"
        ...
        return (
            <>
                <Header
                    sub
                    className="text-xs m-0 flex items-center justify-between w-full"
                >
                    <div className="flex items-center justify-between w-full">
                        <Button
                            disabled={voutFocus === 0}
                            icon="left chevron"
                            size="mini"
                            onClick={() =>
                                setVoutFocus((voutFocus) => voutFocus - 1)
                            }
                        />
                        <div className="text-xs text-gray-600 mr-1">
                            VOUTs {voutFocus + 1} of {totalPages}{" "}
                        </div>
                        <Button
                            disabled={voutFocus >= totalPages - 1}
                            icon="right chevron"
                            size="mini"
                            onClick={() =>
                                setVoutFocus((voutFocus) => voutFocus + 1)
                    ...
                    <div>
                        <Label
                            ...
                            color={
                                get(txObj, ["vouts", voutFocus, "type"]) ===
                                "ValueStore"
                                    ? "green"
                                    : "purple"
                            }
    ...
    return (
        <Page showMenu showNetworkStatus>
            <Grid textAlign="center" className="m-0" container>
                <Grid.Column
                    width={16}
                    className="p-0 self-center text-md"
                    style={{ height: "370px" }}
                >
                    {txObj.error ? (
                        ...
                        <>
                            ...
                            <div
                                ...
                                onClick={() =>
                                    window.open(
                                        `${BLOCK_EXPLORER_LINK}/tx?txHash=${txObj["txHash"]}`,
                                        "_blank"
                                    )
                                }
                            >
