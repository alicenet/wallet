...
function EnterRecoveryPhrase() {
    ...
    const [verifyPhraseButtonText, setVerifyPhraseButtonText] =
        React.useState("Use This Phrase");
    const [seedPhrase, setSeedPhrase] = React.useState(
        (storedSeedPhrase || []).join(" ")
    );
    ...
    React.useEffect(() => {
        setSeedPhraseIsCorrect(
            parsedSeedPhrase.length === 12 &&
                uniq(parsedSeedPhrase).length === 12 &&
                parsedSeedPhrase.every(
                    (phrase) => phrase.length > 2 && phrase.length < 9
                )
        );
    }, [parsedSeedPhrase]);
    ...
    return (
        <Page>
            <Grid textAlign="center" className="m-0">
                ...
                <Grid.Column width={16} className="p-0 self-center">
                    <p>
                        Enter a seed and select deterministic wallets to import
                        from that seed.
                        <br />
                        These accounts will be added to your current wallet
                        vault.
                    </p>
                </Grid.Column>
                ...
                <Grid.Column width={16} className="p-0 self-center">
                    <p>
                        Only attempt imports of seeds that you have used this
                        derivation path with for results you expect.
                    </p>
                </Grid.Column>
                ...
                <Grid.Column width={12} className="p-0 self-center">
                    <Container>
                        ...
                        <p className="text-xs">
                            {" "}
                            Please note the derivation path: m/44'/0'/0'/0/0
                            will be used.
                        </p>
                    </Container>
                </Grid.Column>
                ...
                <Grid.Column width={12} className="p-0 self-center">
                    <Container
                        fluid
                        className="flex flex-wrap gap-2 text-left max-h-24 overflow-y-auto overscroll-auto"
