...
function GetNewSeedPhrase() {
    ...
    const { seedPhrase } = useSelector((state) => ({
    ...
    const rollPotentialSeedPhrase = React.useCallback(
        () => dispatch(USER_ACTIONS.setNewPotentialMnemonic()),
        [dispatch]
    );
    ...
    return (
        <Page>
            <Grid textAlign="center" className="m-0">
                ...
                <Grid.Column width={16} className="p-0 self-center">
                    <p>
                        Below is your seed phrase, please store it in a secure
                        place.
                    </p>
                ...
                <Grid.Column width={12} className="p-0 self-center">
                    <Grid celled columns={6} className="m-0">
                        {chunk(
                            seedPhraseSplitted,
                            Math.floor(seedPhraseSplitted.length / 2)
                        ).map((someSeeds, index) => (
                            <GridRow key={`seed-row-${index}`}>
                                {someSeeds.map((word, position) => (
                                    <Grid.Column
                                        key={`seed-column-${position}`}
                                    >
                                        {word}
                                    </Grid.Column>
                                ))}
                ...
                <Grid.Column width={16} className="p-0 self-center">
                    ...
                    <Button
                        ...
                        size="tiny"
                        icon="redo"
                        content="Reroll Phrase"
                        onClick={rollPotentialSeedPhrase}
                    />
                </Grid.Column>
                    ...
                    <Container className="flex justify-between">
                        <div>
                            <Button
                                ...
                                content="Go Back"
                                onClick={() =>
                                    history.push("/newVault/createVault")
                                }
                            />
                        </div>
                        ...
                        <div className="flex flex-col gap-2">
                            <Button
                                ...
                                onClick={() =>
                                    history.push("/newVault/verifySeedPhrase")
                                }
                            />
                            ...
                            <Checkbox
                                ...
                                label={
                                    <label className="text-sm">
                                        I Have Stored This Seed Securely
                                    </label>
                                }
                            />
                        </div>
