...
export default function GenerateKeystoreForm({
    ...
    const generateWallet = async () => {
        let newStoreBlob = await utils.wallet.generateKeystore(
            true,
            formState.password.value,
            curveType
        );
        ...
        return (
            ...
            >
                ...
                <Form.Input
                    label={
                        <>
                            <label className="inline text-left">
                                Wallet Name
                            </label>
                            <Popup
                                size="mini"
    ...
    return (
        <Form size="mini" className="w-96 mb-12 mini-error-form text-left">
            ...
            <Form.Input
                ...
                type={showPassword ? "string" : "password"}
                value={formState.verifiedPassword.value}
                onChange={(e) => formSetter.setVerifiedPassword(e.target.value)}
                action={{
                    content: "Generate",
                    size: "mini",
                    onClick: () => onSubmit(generateWallet),
                    icon: "refresh",
                    className: "w-28",
                }}
                error={
