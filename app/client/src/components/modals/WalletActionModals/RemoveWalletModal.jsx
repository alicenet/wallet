...
export default function RemoveWalletModal() {
    ...
    const { isOpen, targetWallet, exists, optout, wallets } = useSelector(
        (s) => ({
    ...
    const removeWallet = async () => {
        ...
        try {
            ...
            let deleteWallet = await dispatch(
                VAULT_ACTIONS.removeWalletByAddress(
                    targetWallet,
                    formState.password.value,
                    optout,
                    exists
            ...
            if (deleteWallet.error) {
                ...
                closeModal();
                const walletHead =
                    head(wallets.internal) || head(wallets.external);
                ...
                <div>
                    <div className="p-4">
                        ...
                        <Modal.Content className="text-sm">
                            {exists ? (
                                <>
                                    <p>
                                        Removing a wallet is considered an
                                        administrative action.
                                    </p>
                                    <p>
                                        Please provide your vault password below
                                        to confirm this removal.
                                    </p>
                                </>
                            ) : (
                                <p>Please confirm before deleting</p>
                            )}
                            ...
                            {error && (
                                <Message color={error ? "red" : "purple"}>
                                    {error}
                                </Message>
                            )}
                        </Modal.Content>
                    ...
                    <Modal.Actions className="border-solid border-t-1 border-b-0 border-l-0 border-r-0 border-gray-300 bg-gray-100 p-4">
                        <div className="flex justify-between">
                            ...
                            <Button
                                size="small"
                                content={error ? "Try Again" : "Delete Wallet"}
                                color="teal"
