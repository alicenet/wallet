...
export default function NetworkStatusIndicator() {
    ...
    const [formState, formSetter, onSubmit] = useFormState([
        {
            name: "AliceNetProvider",
            display: "AliceNet Provider",
            type: "url",
            isRequired: true,
            value: aliceNetProvider,
        },
        {
            name: "EthereumProvider",
            display: "Ethereum Provider",
            type: "url",
            isRequired: true,
        ...
        },
        {
            ...
            display: "Registry Contract Address",
            type: "string",
            isRequired: true,
    ...
    if (
        ...
        registryContractError =
            "Ethereum errors can related to the Registry Address, please verify it.";
    }
    if (
        web3Error &&
        web3Error.indexOf(
            "is invalid, the capitalization checksum test failed"
        ) !== -1
    ) {
    ...
    const aliceNetColor = aliceNetBusy
        ? "yellow"
        : aliceNetConnected
        ? "green"
        : "red";
    ...
    const notifyError = (message) => {
        toast.error(
            <SyncToastMessageWarning title="Error" message={message} />,
            { autoClose: 2000 }
        );
    ...
    const notifySuccess = (message) => {
        toast.success(
            <SyncToastMessageSuccess title="Success" message={message} />,
            { autoClose: 1000 }
        );
        dispatch(INTERFACE_ACTIONS.toggleGlobalLoadingBool(false));
    };
                ...
                >
                    <div
                        className={classNames(
                            ...
                            { "animate-pulse text-red-600": aliceNetError }
                        )}
                    >
                        <div className="font-bold font-mono">ALI</div>
                        <div
                            className="relative"
                            style={{ top: "-1px", marginLeft: "4px" }}
                        >
                    ...
                    <div
                        className={classNames(
                            "flex items-center text-gray-600",
                            { "animate-pulse text-red-600": web3Error }
                        )}
                    >
                        <div className="font-bold font-mono">ETH</div>
                        <div
                            className="relative"
                            style={{ top: "-1px", marginLeft: "4px" }}
                        >
        ...
        >
            ...
            <Modal.Content className="flex flex-col text-xs">
                <Grid textAlign="center" className="m-0 content-evenly">
                    <Grid.Column
                        width={12}
                        className="self-center p-0 m-0 text-left"
                    >
                        <Form className="text-sm">
                            <Form.Input
                                ...
                                onChange={(e) =>
                                    formSetter.setAliceNetProvider(
                                        e.target.value
                                    )
                                }
                                error={
                                    (!!formState.AliceNetProvider.error && {
                                        content:
                                            formState.AliceNetProvider.error,
                                    }) ||
                                    aliceNetError
                            ...
                            <Form.Input
                                ...
                                onChange={(e) =>
                                    formSetter.setEthereumProvider(
                                        e.target.value
                                    )
                                }
                                error={
                            ...
                            <Form.Input
                                ...
                                onChange={(e) =>
                                    formSetter.setRegistryContractAddress(
                                        e.target.value
                                    )
                                }
                                error={
                                    (!!formState.RegistryContractAddress
                                        .error && {
                                        content:
                                            formState.RegistryContractAddress
                                                .error,
...
function StatusLight({ className, color }) {
    let baseClass = classNames("rounded-full border-solid border", {
        className: className,
    });
    let colorClass =
        color === "red"
